    ...
    (testing "when there are more commits then limit specified"
      (let [result (compare-commits "rails" "jbuilder" "v2.5.0" "v2.6.0")
            commits (->> (result :commits)
                         (mapv (fn [{:keys [html_url commit]}] {:url html_url :message (commit :message)})))]
        (is
         (and
          (= commits [
                      ...
                      {:url "https://github.com/rails/jbuilder/commit/fc6fa813c6bf8b593075f116b0e9047a0e4ddff7",
                       :message
                       "Use fragment_cache_key when available so caches are properly expired"}
                      {:url "https://github.com/rails/jbuilder/commit/c26ad3596fb84d16f9e21b3ce394c3b4377c2b6b",
                       :message "Remove relic and confusing  argument"}])
          (= (result :status) "ahead"))))))
